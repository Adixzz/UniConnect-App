...
class _MeetingDetailsScreenState extends State<MeetingDetailsScreen> {
      ...
      if (response.statusCode == 200) {
        ...
        for (int i = 1; i < sheet.length; i++) {
          ...
          for (int j = 2; j < dates.length; j++) {
            ...
            if (dateString.isEmpty || weekendColumnIndices.contains(j))
              continue;
            ...
            try {
              ...
              if (fullDate.isAtSameMomentAs(todayMidnight) ||
                  (fullDate.isAfter(todayMidnight) &&
                      fullDate.isBefore(
                        rangeLimit.add(const Duration(seconds: 1)),
                      ))) {
  ...
  Widget _buildDateFilterBar() {
    if (uniqueDates.isEmpty) {
      return const Text(
        "No available dates found for this lecturer.",
        style: TextStyle(color: Colors.grey),
      );
    }
    String todayMarker = DateFormat(
      "MMM d",
